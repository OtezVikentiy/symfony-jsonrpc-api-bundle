...
### Изменено
- **Публичным свойствам Request DTO больше не нужны геттер и сеттер.** Promoted-свойства гидрируются через конструктор, остальные публичные свойства записываются напрямую. Для private и protected сохраняется прежний контракт аксессоров, строгая проверка объявленных типов не меняется.
