...
Функциональный релиз: JSON-RPC методы теперь могут принимать загрузку файлов через `multipart/form-data`, opt-in и выключено по умолчанию. Вклад @tacman (#9), в форме, согласованной в #8. Строго обратно совместим: с выключенной функцией (по умолчанию) поведение идентично 5.0.
