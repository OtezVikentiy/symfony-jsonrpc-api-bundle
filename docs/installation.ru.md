...
У бандла есть официальный [contrib-рецепт Symfony Flex](https://github.com/symfony/recipes-contrib). Если проект использует Flex, рецепт выполнит шаги 2 и 3 за вас: подключит бандл в `config/bundles.php`, создаст `config/packages/ov_json_rpc_api.yaml` и `config/routes/ov_json_rpc_api.yaml` и добавит переменные окружения в `.env`. Flex запрашивает подтверждение перед выполнением contrib-рецепта; чтобы разрешить contrib-рецепты для проекта заранее, выполните один раз:

```bash
composer config extra.symfony.allow-contrib true
```

С Flex можно сразу переходить к просмотру сгенерированных файлов — разделы ниже в этом случае служат справочником по ключам конфигурации. Без Flex выполните шаги 2 и 3 вручную.
...
## 3. Создайте конфигурационные файлы (при использовании Symfony Flex — проверьте сгенерированные)
...
- [Базовый пример создания API-метода](./examples/base.ru.md)
- [Генерация Swagger-документации](./swagger/tags.ru.md)
- [Настройка безопасности](./security/roles.ru.md)
