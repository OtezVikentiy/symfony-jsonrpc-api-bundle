...
>
> Сериализация ответа устроена **тем же** механизмом (общий трейт), с одним отличием: вложенный объект со своим `toArray()` в запросе решает свою форму сам, а в ответе — нет, там форму задают геттеры. См. [примечание о геттерах в базовом примере](./examples/base.ru.md#response) и [upgrade-5.0.md](./upgrade-5.0.md).
