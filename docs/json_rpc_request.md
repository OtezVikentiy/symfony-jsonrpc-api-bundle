...
> Это про `JsonRpcRequest::toArray()` — вспомогательный метод для ваших Request-DTO. Сериализация самого **ответа** (`Response`-класса, который возвращает `call()`) устроена отдельно — см. [примечание о геттерах в базовом примере](./examples/base.md#response) и [upgrade-5.0.md](./upgrade-5.0.md).
