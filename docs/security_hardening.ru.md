...
> **Единственное исключение, и оно включается дважды: загрузка файлов через `multipart/form-data`.** При `multipart.enabled: true` *и* `acceptsMultipart: true` у метода этот метод принимает multipart — а вместе с ним и описанную выше CSRF-экспозицию, потому что `multipart/form-data` — такой же CORS «simple request», как и form-encoded. Оба переключателя выключены по умолчанию, поэтому ничего не меняется, пока их не включат осознанно, а метод без флага по-прежнему отклоняется с `-32600`. Что проверить перед включением — в [docs/multipart.ru.md](./multipart.ru.md).
...
См. также [docs/cors.md](./cors.ru.md).
...
- [docs/cors.md](./cors.ru.md) — детальное поведение CORS, preflight, credentials
- [docs/upgrade-4.0.md](./upgrade-4.0.md) — миграция с 3.x на 4.0
- [docs/batch.md](./batch.ru.md) — поведение batch-запросов
